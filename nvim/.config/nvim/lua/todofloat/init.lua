...
local function open_floating_file(target_file)
	...
	if vim.fn.filereadable(expanded_path) == 0 then
		vim.notify("To-do file not found: " .. expanded_path, vim.log.levels.ERROR)
		return
	end
	...
	vim.api.nvim_buf_set_keymap(buf, "n", "q", "", {
		...
		callback = function()
			if vim.api.nvim_get_option_value("modified", { buf = buf }) then
				vim.notify("Please save changes before closing.", vim.log.levels.WARN)
			else
...
local function setup_user_commands(opts)
	local targer_file = opts.target_file or "todo.md"
	vim.api.nvim_create_user_command("Td", function()
		open_floating_file(targer_file)
	end, {})
end
